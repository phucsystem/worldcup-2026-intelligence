...
type BackgroundFlagEntry = {
  flag: string;
  label: string;
};

const BACKGROUND_FLAGS: Record<string, string> = {
...
const PARTICIPANT_FLAGS: BackgroundFlagEntry[] = [
  { flag: "ca", label: "CAN" },
  { flag: "us", label: "USA" },
  { flag: "mx", label: "MEX" },
  { flag: "br", label: "BRA" },
...
function BackgroundFlag({ flag, label, index }: BackgroundFlagEntry & { index: number }) {
  const tilt = `${((index % 7) - 3) * 3}deg`;
  const lift = `${(index % 6) * 5}px`;
  const svg = BACKGROUND_FLAGS[flag];

  if (!svg) {
    return (
      <span
        aria-hidden="true"
        className="site-flag crest"
        style={
          {
            "--tilt": tilt,
            "--lift": lift,
            backgroundColor: "#1E3157",
          } as CSSProperties
        }
      >
        {label}
  ...
  return (
    <span
      aria-hidden="true"
      className="site-flag"
      style={{ "--tilt": tilt, "--lift": lift } as CSSProperties}
      dangerouslySetInnerHTML={{
        __html: `<svg viewBox="0 0 30 20" class="flag-svg" role="img" aria-label="${label} flag">${svg}</svg>`,
      }}
    />
  );
}
...
export default function SiteBackground() {
  return (
    <div className="site-background" aria-hidden="true">
      {PARTICIPANT_FLAGS.map((entry, index) => (
        <BackgroundFlag key={`${entry.label}-${index}`} {...entry} index={index} />
      ))}
    </div>
  );
}
